...
class HuffmanSuite {
  ...
  @Test def `time returns List`: Unit =
    new TestTrees {
      assertEquals(List(('a',2),('b',3)), times(List('b','a','b','a','b')))
    }

  @Test def `makeOrderedLeafList returns Leaf`: Unit =
    new TestTrees {
      assertEquals(List(Leaf('c',1),Leaf('a',2),Leaf('b',3)), makeOrderedLeafList(List(('b',3),('a',2),('c',1))))
    }
  ...
  @Test def `combine of some leaf list (15pts)`: Unit = {
   ...
   @Test def `decode simple tree`: Unit =
    new TestTrees {
